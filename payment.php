...
$getSelectedItemsQuery = "SELECT products.ItemID, products.ItemName, products.price, cart.quantity 
                         FROM cart 
                         INNER JOIN products ON cart.ItemID = products.ItemID 
                         WHERE cart.customer_id = ? AND cart.cart_id IN ($selectedItems)";

$stmtGetSelectedItems = mysqli_prepare($conn, $getSelectedItemsQuery);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmtSaveOrder = mysqli_prepare($conn, $saveOrderQuery);
    ...
    if ($stmtSaveOrder) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $stmtUpdate = mysqli_prepare($conn, $updateOrderQuery);
            ...
            if ($stmtUpdate) {
                ...
                echo "Error updating order with product IDs: " . mysqli_error($conn);
            }
        }
    ...
    $stmtSavePayment = mysqli_prepare($conn, $savePaymentQuery);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $updateitemsQuery = "UPDATE products SET quantity = quantity - ?, Solds = Solds + ? WHERE ItemID = ?";
        $stmtUpdateitems = mysqli_prepare($conn, $updateitemsQuery);
        ...
        if ($stmtUpdateitems) {
            ...
            echo "Error updating quantity and sold count: " . mysqli_error($conn);
        }
    }
    ...
    if ($stmtUpdateOrder) {
        ...
        echo "Error updating order with product IDs and quantities: " . mysqli_error($conn);
    }
